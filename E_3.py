...
#ig = cv2.imread('Dibujo.jpeg',0)
im = cv2.imread('Dibujo.jpeg')
ig = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
[M,N] = ig.shape[0:2] #calcular tamaño de la imagen

hist = cv2.calcHist([ig],[0],None,[4],[0,256]).flatten()/(M*N) 
#Para calcular cuartiles, solo ponemos un 4 en el 4to parámetro
...
if maxElement == 3 and hist[3] > 0.3: #en programación comienzan
